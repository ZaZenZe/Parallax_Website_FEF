...
const Facts = () => {
  return (
    <section className="facts" id="facts">
      ...
      >
        <span className="facts__eyebrow">Lore fragments</span>
      ...
      <div className="facts__grid">
        {facts.map((fact, index) => (
          ...
          >
            <header className="facts__card-header">
              <span className="facts__badge">{fact.id}</span>
              <span className="facts__highlight">{fact.highlight}</span>
            </header>
            <h3 className="facts__card-title">{fact.title}</h3>
            <p className="facts__card-text">{fact.description}</p>
          </motion.article>
        ))}
      </div>
